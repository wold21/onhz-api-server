...
public class TrackScheduleService implements RatingScheduleInterface {
        ...
        for (Object entity : entities) {
            try {
                ...
                if (reviewCount <= BATCH_SIZE) {
                    log.info("트랙 {} _ 리뷰 수 {}건(1000건 이하)이므로 이미 업데이트 수행됨", entityId, reviewCount);
                    return;
                }
    ...
    public void updateSummaryImmediately(Long entityId) {
        long reviewCount = reviewRepository.countByReviewTypeAndEntityIdAndRatingIsNotNull(ReviewType.TRACK, entityId);
        if (reviewCount <= BATCH_SIZE) {
            log.info("트랙 {} _ 리뷰 수 {}건(1000건 이하)이므로 즉시 Summary 테이블 업데이트 수행", entityId, reviewCount);
            entityInsertAndUpdate(entityId);
        }
    }
}
