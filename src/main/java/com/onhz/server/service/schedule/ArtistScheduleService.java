...
public class ArtistScheduleService implements RatingScheduleInterface {
        ...
        for (Object entity : entities) {
            try {
                ...
                if (reviewCount <= BATCH_SIZE) {
                    log.info("아티스트 {} _ 리뷰 수 {}건(1000건 이하)이므로 이미 업데이트 수행됨", entityId, reviewCount);
                    return;
                }
    ...
    public void updateSummaryImmediately(Long artistId) {
        long reviewCount = reviewRepository.countByReviewTypeAndEntityIdAndRatingIsNotNull(ReviewType.ARTIST, artistId);
        if (reviewCount <= BATCH_SIZE) {
            log.info("아티스트 {} _ 리뷰 수 {}건(1000건 이하)이므로 즉시 Summary 테이블 업데이트 수행", artistId, reviewCount);
            entityInsertAndUpdate(artistId);
        }
    }
}
